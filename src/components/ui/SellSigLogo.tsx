...
export function SellSigLogo({
  ...
  const logoContent = <div className={cn("flex items-center gap-2.5 hover:opacity-90 transition-opacity group", className)}>
      <div className={cn("rounded-xl flex items-center justify-center backdrop-blur-sm border group-hover:border-opacity-60 transition-colors", sizes.container, colors.containerBg, colors.containerBorder)}>
        <SellSigIcon className={sizes.icon} />
      </div>
      <div className="flex flex-col">
        <span className={cn("font-bold tracking-tight leading-none", sizes.text, colors.text)}>SELLSIG<span className={colors.highlight}>SIG</span>
        </span>
        {showTagline && <span className={cn("font-medium tracking-widest uppercase leading-none mt-4.5", sizes.tagline, colors.tagline)}>
            AI Coach
          </span>}
      </div>
    </div>;
